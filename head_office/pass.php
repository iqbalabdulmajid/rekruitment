...
if (isset($_POST["simpan"])) {
	...
	$sql = "SELECT * FROM head_office WHERE id_head_office = '$id_head_office'";
	...
	echo "<meta http-equiv='refresh' content='1;URL=pass.php?id_admin=$id_head_office'>";
	
	} elseif ($password_baru_1 != $password_baru_2) {
	
	echo "<script>alert(\"Konfirmasi password baru tidak sama!\");</script>";
	echo "<meta http-equiv='refresh' content='1;URL=pass.php?id_head_office=$id_head_office'>";
	
	} elseif ($password_sekarang == $admin["password"] && $password_baru_1 == $password_baru_2) {
	
	$sql = "UPDATE admin SET password = '$password_baru_1' WHERE id_head_office = '$id_head_office'";
	$query = mysqli_query($connect,$sql);
	if (!$query){
