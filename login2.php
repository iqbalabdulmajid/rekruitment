...
<?php 
	...
	if (@$_POST['login']) {
        ...
        else if($level == 'head_office'){

			$sql2 	= mysqli_query($connect, "SELECT * FROM head_office where username='$username' AND password='$password'");
			$data2	= mysqli_fetch_array($sql2);
			$id2 	= $data2[0];
			$cek2	= mysqli_num_rows($sql2);
			if ($cek2 > 0) {
				$_SESSION['head_office'] = $id2;
				header("location: head_office/index.php");
			} else{
				echo "<script>alert('login admin gagal, silahkan ulang kembali')</script>";
			}
